...
class ScheduleFragment : Fragment() {
    ...
    private fun onToggleStarState(session: Session): Boolean {
        val sessionIds = PreferenceUtil.loadStarredIds1(mActivity).toMutableList()
        ...
        PreferenceUtil.saveStarredIds1(mActivity, sessionIds)
        return !isAlreadyStarred
    }

    private fun isSessionStarred(session: Session): Boolean {
        return PreferenceUtil.loadStarredIds1(mActivity).contains(session.id)
    }

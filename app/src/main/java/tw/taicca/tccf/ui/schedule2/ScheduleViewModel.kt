...
import tw.taicca.tccf.extension.debounce
import tw.taicca.tccf.model.ConfSchedule
import tw.taicca.tccf.model.Session
import tw.taicca.tccf.model.SessionTag
...
class ScheduleViewModel(application: Application, stateHandle: SavedStateHandle) : AndroidViewModel(application) {
    ...
    private suspend fun getSchedule() = withContext(Dispatchers.Default) { PreferenceUtil.loadSchedule2(getApplication()) }
    ...
    private fun filterStarred(sessions: Map<String, List<Session>>): Map<String, List<Session>> {
        val starredIds = PreferenceUtil.loadStarredIds2(getApplication())

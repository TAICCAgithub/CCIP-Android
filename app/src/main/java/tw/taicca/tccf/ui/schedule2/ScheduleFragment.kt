package tw.taicca.tccf.ui.schedule2
...
import tw.taicca.tccf.extension.doOnApplyWindowInsets
import tw.taicca.tccf.extension.dpToPx
import tw.taicca.tccf.extension.updateMargin
import tw.taicca.tccf.model.Session
import tw.taicca.tccf.ui.sessiondetail.SessionDetailActivity
import tw.taicca.tccf.util.AlarmUtil
...
class ScheduleFragment : Fragment() {
    ...
    private fun onToggleStarState(session: Session): Boolean {
        val sessionIds = PreferenceUtil.loadStarredIds2(mActivity).toMutableList()
        ...
        PreferenceUtil.saveStarredIds2(mActivity, sessionIds)
        return !isAlreadyStarred
    }

    private fun isSessionStarred(session: Session): Boolean {
        return PreferenceUtil.loadStarredIds2(mActivity).contains(session.id)
    }
